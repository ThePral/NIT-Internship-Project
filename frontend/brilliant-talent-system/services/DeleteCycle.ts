...
export async function DeleteCycle(id: number) {
    const result = await deleteFetch(APIURL + `cycle/${id}`);

    const jsonResult = await result.json();

    if (result.ok) {
        toast.success("دوره با موفقیت حذف شد", {
            description: "دوره انتخاب‌شده با موفقیت از سیستم حذف گردید.",
        });
        return jsonResult;
    } else {
        ...
        switch (result.status) {
            case 400:
                toast.error("خطای درخواست", {
                    description: jsonResult.message || "شناسه دوره نامعتبر است.",
                });
                break;
            case 401:
                toast.error("عدم دسترسی", {
                    description: "شما مجوز حذف دوره را ندارید.",
                });
                break;
            case 403:
                toast.error("ممنوع", {
            ...
            case 409:
                toast.error("تضاد", {
                    description:
                        "امکان حذف این دوره وجود ندارد (دوره در حال استفاده است).",
                });
                break;
            case 500:
