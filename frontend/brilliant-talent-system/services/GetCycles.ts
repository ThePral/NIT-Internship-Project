...
export async function GetCycles(): Promise<Cycle[]> {
    try {
        const result = await getFetch(APIURL + `cycle`);
        const jsonResult = await result.json();

        if (result.ok) {
            console.log("لیست دوره ها با موفقیت دریافت شد", jsonResult);
            return jsonResult;
        }

        let message = "مشکلی در دریافت لیست دوره ها رخ داده است.";

        switch (result.status) {
            case 400:
                message =
                    jsonResult.message || "درخواست نامعتبر برای دریافت لیست دوره ها.";
                toast.error("خطای درخواست", { description: message });
                break;
            case 401:
                message = "برای مشاهده لیست دوره ها باید وارد حساب کاربری شوید.";
                toast.error("عدم دسترسی", { description: message });
                break;
            case 403:
                message = "شما مجوز دسترسی به لیست دوره ها را ندارید.";
                toast.error("ممنوع", { description: message });
                break;
            case 404:
                message = "هیچ دوره ای در سیستم یافت نشد.";
                toast.error("یافت نشد", { description: message });
                break;
            case 409:
                message = "مشکل در دریافت لیست دوره ها رخ داده است.";
                toast.error("تضاد", { description: message });
                break;
            case 500:
                message = "خطایی در دریافت لیست دوره ها رخ داده است.";
